...
module.exports.calcMatches = function (dameId, sireId) {
  return axios.post('https://dna.cryptokittydata.info/fetch/dna', {
    ...
    let dameDna = transformDNA(response.data[dame].dna);
    let sireDna = transformDNA(response.data[sire].dna);

    return compareDna(dameDna, sireDna)
  });
};

module.exports.getProfileDna = function (profile) {
...
module.exports.findMatchesForGeneration = function (generation, profile) {
  return this.getProfileDna(profile).then( () => {
    let filtered = [];
    let topMatches = [];
...
//
// let dame = process.argv[2];
// let sire = process.argv[3];
//returnMatch(dame, sire).then(matches => { console.log(matches) });